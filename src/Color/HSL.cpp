...
namespace ysImageProcessing {
	namespace Color {
		...
		HSL::HSL(const float& t_h, const float& t_s, const float& t_l) :
		m_h(t_h), m_s(t_s), m_l(t_l) {
		}
		...
		float HSL::HUE2RGB(float t_p, float t_q, float t_t) {
			if (t_t < 0.0f) t_t += 1.0f;
			if (t_t > 1.0f) t_t -= 1.0f;
			if (6.0f * t_t < 1.0f) return t_p + (t_q - t_p) * 6.0f * t_t;
			if (2.0f * t_t < 1.0f) return t_q;
			if (3.0f * t_t < 2.0f) return t_p + (t_q - t_p) * (2.0f / 3.0f - t_t) * 6.0f;
			return t_p;
		}
